...
test('manual executor emits exactly the engine selection (no drop/alter, spec #20)', async () => {
  ...
  assert.deepEqual(res.selections, await buildSelections(approved, resolver));
  await assertSelectionFidelity(approved, resolver, res.selections);
});
...
test('buildSelections maps ApprovedPick -> share selection spec', async () => {
  const picks = [{ matchId: 'E1', marketId: '18', selection: 'Over 2.5', line: 2.5 }];
  const sels = await buildSelections(picks, resolver);
