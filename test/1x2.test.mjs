...
test('boundary: 1.49 reject, 1.50 accept, 1.77 accept, 2.19 accept, 2.20 reject', () => {
  ...
  const picks = select1X2Picks(buildFavRows(d), 1.5, 2.2);
  assert.deepEqual(
...
test('O/U, correct-score, totals are never selected (only 1X2 favorite)', () => {
  ...
  const picks = select1X2Picks(buildFavRows(dbOf(e)), 1.5, 2.2);
  assert.equal(picks.length, 0);
});

test('shared selector is identical for paper and manual paths', () => {
  // Both call select1X2Picks(buildFavRows(db), lo, hi) -> same set by construction.
  const d = dbOf(
    ev('p1', 1.67, 3.4, 4.0),
    ev('p2', 2.15, 3.4, 4.0),
  ...
  const lo = 1.5;
  const hi = 2.2;
  const paper = select1X2Picks(buildFavRows(d), lo, hi);
  const manual = select1X2Picks(buildFavRows(d), lo, hi);
  assert.deepEqual(
