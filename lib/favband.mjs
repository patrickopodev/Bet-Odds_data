...
// The validated 1X2 favorite-value band. Single source of truth: read from the
// frozen strategy registry (STRAT-1X2-FAVBAND-v1). Hardcoded fallback keeps the
// system safe if the registry is missing. NO env override — a validated
// strategy must never be silently widened by a deployment variable (review
// action #1: eliminate the 1.5/1.8 discrepancy).
export const FROZEN_FAV_BAND = Object.freeze({ lo: 1.8, hi: 2.2 });

export function frozenFavBand() {
  try {
    const reg = JSON.parse(fs.readFileSync(REGISTRY_PATH, 'utf8'));
    const s = reg.strategies?.find((x) => x.strategyId === 'STRAT-1X2-FAVBAND-v1');
    if (s?.parameters?.lo != null && s?.parameters?.hi != null) {
      return Object.freeze({ lo: Number(s.parameters.lo), hi: Number(s.parameters.hi) });
    }
  } catch {
    /* fall through to fallback */
  }
  return FROZEN_FAV_BAND;
}
