...
export const TEAM_CACHE_FILE = path.join(DATA_DIR, 'team-cache.json');
...
// Retry wrapper with exponential backoff for transient errors (403,
// network timeouts, etc.). Used by fetchJson and fetchText.
async function fetchWithRetry(fn, retries = 3, baseDelay = 2000) {
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (!isRetriable) throw lastErr;
      const delay = baseDelay * Math.pow(2, i);
      console.warn(`[retry] attempt ${i + 1}/${retries + 1} failed: ${e.message}; waiting ${delay}ms`);
...
export async function fetchSportyApiJson(url, options = {}) {
  // Transient 403/timeout errors are handled by fetchJson's retry loop.
...
// SportyBet's betting day starts at 00:00 local (UTC+0 for GH): every match
// kicking off at or after midnight belongs to the NEW day, so the "today" list
// is a strict 24h window from local midnight to the next local midnight. Any
// overnight match (e.g. 00:30 kickoff) is shown by SportyBet under tomorrow's
// date, not today's.
export const SPORTYBET_TODAY_ROLLOVER_HOUR = 0;
...
export async function fetchTodayFootballEvents() {
  ...
  const dayEnd = new Date(dayStart.getTime() + (24 + SPORTYBET_TODAY_ROLLOVER_HOUR) * 60 * 60 * 1000);
...
// Missing markets come back as null. All API markets are now collected (no filter).
export async function fetchEventMarketsByKey(eventId, productId = SPORTYBET_PRODUCT_ID) {
  ...
  for (const [id, m] of Object.entries(merged)) {
    ...
    if (key) {
      ...
      // Market ID not in TARGET_MARKET_IDS — store under its own ID
      byKey[id] = { marketId: id, name: m.name, group: m.group, outcomes: m.outcomes };
    }
  }
  // Return ALL markets: known sections from MARKET_ORDER + any extra market IDs
  const allKeys = [...MARKET_ORDER, ...Object.keys(byKey).filter((k) => !MARKET_ORDER.includes(k))];
  return Object.fromEntries(allKeys.map((k) => [k, byKey[k]]));
}

// Prune old snapshot files from DATA_DIR, keeping only the most recent
// `keep` snapshots (and always preserving latest.json). Returns the number
// of files removed so callers can log the cleanup.
export async function pruneSnapshots(keep = 30) {
  const dir = await fs.mkdir(DATA_DIR, { recursive: true });
  const files = await fs.readdir(DATA_DIR);
  const snapshots = files
    .filter((f) => /^snapshot-\d{4}-\d{2}-\d{2}T/.test(f) && f.endsWith('.json'))
    .sort()
    .reverse();
  const removed = snapshots.length > keep ? snapshots.slice(keep) : [];
...
// Load and validate the v1 database from a JSON file. Rejects if the
// schema is not v1 (missing version or events), preventing drift.
export async function loadDb(dbFile = DB_FILE) {
  const raw = await fs.readFile(dbFile, 'utf8');
  const db = JSON.parse(raw);
  if (db.version !== 1 || !db.events) {
    throw new Error('not a v1 database');
  }
  return db;
}

// Compact the odds-db.json by collapsing identical-odds repeats within a
// single event's play history into one play with the earliest seenAt. This
// bounds DB growth when the same price has been scraped many times.
export async function compactDb(dbFile = DB_FILE) {
  const db = await loadDb(dbFile);
  let compacted = 0;
  for (const ev of Object.values(db.events ?? {})) {
    for (const out of Object.values(ev.outcomes ?? {})) {
      ...
      // Group by distinct odds; keep the earliest seenAt for each.
      const best = new Map(); // odds -> earliest seenAt
      for (const p of plays) {
        const prev = best.get(p.odds);
        if (!prev || p.seenAt < prev) best.set(p.odds, p.seenAt);
      }
      const newPlays = [...best].map(([odds, seenAt]) => ({ odds, active: true, seenAt }));
      if (newPlays.length < plays.length) {
        out.plays = newPlays;
        compacted++;
      }
    }
  ...
  if (compacted > 0) {
    ...
    await saveDb(db, dbFile);
  }
  return compacted;
}

// SaveDb now prunes snapshots and compacts plays on every write.
export async function saveDb(db, dbFile = DB_FILE) {
  ...
  // Cleanup: drop old snapshots beyond the keep window; compact duplicate plays.
  await pruneSnapshots(30).catch(() => {});
  await compactDb(dbFile).catch(() => {});
  return true;
}

// Simulated Reality Leagues ("SRL") are SportyBet's virtual simulations of
// real teams — not real football. They must never be staked: the odds move by
// simulation, not by events on a pitch. Matched against tournament AND team
// names (some listings carry the Srl tag on the team only).
export function isSimulated(name) {
  return /\bsrl\b/i.test(String(name ?? ''));
}

// A match is a friendly when its tournament (or either team name) carries a
// friendly / pre-season tag. Centralized here so the engine's LIVE selection and
// the legacy staking path (`stake.mjs`) apply the SAME friendly gate and can
// never diverge on which matches are stakeable (spec: friendly gate).
export function isFriendly(name) {
  return /friendly|friendlies|pre-season|preseason/i.test(String(name ?? ''));
}
...
// a Flashscore page HTML and return every event block (a decoded field map with
// an AA id). Shared by resolve-results.mjs and the TS agent so the two can never
// drift on how a feed is decoded.
...
// In-memory cache + loaded flag to avoid re-reading team-cache.json
// on every resolveTeam() call. Lazily populated on first invocation.
let teamCache = new Map();
let cacheLoaded = false;

// Load the persistent team resolution cache. Returns a Map of
// normalized team name -> { id, url, name }. Cache is loaded from
// data/team-cache.json if it exists, otherwise empty.
export async function loadTeamCache() {
  try {
    const raw = await fs.readFile(TEAM_CACHE_FILE, 'utf8');
    teamCache = new Map(Object.entries(JSON.parse(raw)));
    return teamCache;
  } catch {
    return new Map();
  }
...
// Save the team resolution cache to disk.
export async function saveTeamCache(cache) {
  const obj = Object.fromEntries(cache);
  await fs.mkdir(path.dirname(TEAM_CACHE_FILE), { recursive: true });
  await fs.writeFile(TEAM_CACHE_FILE, JSON.stringify(obj, null, 2), 'utf8');
}

// Resolve a team's Flashscore page URL + id via the Livesport search API.
// Requires a core-name match (guessing `teams[0]` risks attaching the wrong
// fixture's final score); returns null so the caller skips the event instead.
// Shared by resolve-results.mjs and the TS agent so team resolution never drifts.
// Falls back to: team-cache, Uniscore, in that order.
export async function resolveTeam(name) {
  if (!cacheLoaded) { await loadTeamCache(); cacheLoaded = true; }
  const target = normTeam(name);
  const cached = teamCache.get(target);
  if (cached) return cached;
  // Try Livesport with retry for transient errors
  ...
  try {
    ...
    // Livesport unreachable
  }
  // Fallback to Uniscore if Livesport returned no results or failed
  if (teams.length === 0) {
    const uniscoreResult = await resolveTeamFromUniscore(name);
    if (uniscoreResult) {
      teamCache.set(target, uniscoreResult);
      await saveTeamCache(teamCache);
      return uniscoreResult;
    }
  }
  ...
  if (exact) {
    teamCache.set(target, { id: exact.id, url: exact.url, name: exact.name });
    await saveTeamCache(teamCache);
  }
  return exact ? { id: exact.id, url: exact.url, name: exact.name } : null;
}

// Fallback team resolver using Uniscore search page.
// Fetches the search page and parses HTML for team links.
// Returns { id, url, name } or null.
async function resolveTeamFromUniscore(name) {
  try {
    const q = encodeURIComponent(queryTeam(name));
    const html = await fetchText(`https://uniscore.com/amp/en/search/?q=${q}`);
    // Parse team links from the HTML — Uniscore uses href="/team/{id}/{name}/"
    const teamLinkRegex = /href="\/team\/([^"\/]+)\/([^"]+)"/gi;
    const teamNameRegex = /<[^>]*>\s*([^<]+(?:FC|FK|SC|SS|Club|Academy|Acad)\s*[^<]+)/gi;
    let match;
    const candidates = [];
    while ((match = teamLinkRegex.exec(html)) !== null) {
      const [, id, slug] = match;
      const teamNameMatch = html.substring(Math.max(0, match.index - 200), match.index + 50);
      const nameMatch = teamNameMatch.match(/>([^<]*(?:FC|FK|SC|SS|Club|Academy|Acad|Football)[^<]*)</i);
      candidates.push({ id, url: slug, name: nameMatch ? nameMatch[1].trim() : slug.replace(/-/g, ' ') });
    }
    const target = normTeam(name);
    for (const c of candidates) {
      const n = normTeam(c.name);
      if (n === target || (n.length >= 4 && target.length >= 4 && (n.includes(target) || target.includes(n)))) {
        return c;
      }
    }
  } catch {
    // Uniscore also unreachable
  }
  return null;
}
...
// ---------------------------------------------------------------------------
// Historical performance aggregation: one record per (market, outcome, odds),
// counting each distinct odds value once per match. Shared by analyze-odds.mjs
// and the TS decision agent so the two can never drift on how history is built.
// `winRate` is a number; callers that want display text format it themselves.
// ---------------------------------------------------------------------------
export function aggregateHistoricalStats(db) {
  const stats = new Map(); // `${marketId}|${name}|${odds}` -> aggregate
  for (const ev of Object.values(db.events ?? {})) {
    ...
    // Count each distinct odds value offered for an outcome once per match, so
    // a match scraped across many snapshots isn't counted many times.
    ...
    for (const out of Object.values(ev.outcomes ?? {})) {
      ...
      for (const play of out.plays ?? []) {
        ...
        s.plays++;
        if (result === 'WON') { s.won++; s.settled++; }
        else if (result === 'LOST') { s.lost++; s.settled++; }
        else if (result === 'VOID') { s.void++; s.settled++; }
  ...
  return [...stats.values()].map((s) => ({
    ...
    void: s.void,
    settled: s.settled,
    winRate: s.settled ? s.won / s.settled : null,
