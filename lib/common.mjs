...
export async function fetchSportyApiJson(url, options = {}) {
  // Transient errors (403 rate-limit, network blips) warrant a retry
  // with exponential backoff before the caller gives up.
  const lastError = await fetchWithRetry(async () => {
...
async function fetchWithRetry(fn, retries = 3, baseDelay = 1000) {
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      lastErr = e;
      // Retry on 403 (rate-limit) or network/timeout errors
      const isRetriable =
        i < retries &&
        (e.message.includes('HTTP 403') || e.message.includes('fetch failed') || e.message.includes('timeout'));
